...
func TestValidEdgeCases(t *testing.T) {
    ...
    tt.RunTests(tests, func(subTt *zlsgo.TestUtil, tc zlsgo.TestCase) {
        d := tc.Data.(ValidTestCase)
        got := Valid(d.json)
        subTt.Equal(d.valid, got)
    })
}
...
func TestValidString(t *testing.T) {
    ...
    tests := []zlsgo.TestCase{
        ...
        {Name: "unicode escape", Data: StringTestCase{"\"hello\\u0041world\"", true}},
    ...
    tt.RunTests(tests, func(subTt *zlsgo.TestUtil, tc zlsgo.TestCase) {
        d := tc.Data.(StringTestCase)
        data := []byte(d.input)
        _, ok := validstring(data, 1)
        subTt.Equal(d.valid, ok)
    })
}
...
func TestValidTrue(t *testing.T) {
    ...
    tt.RunTests(tests, func(subTt *zlsgo.TestUtil, tc zlsgo.TestCase) {
        d := tc.Data.(TrueTestCase)
        data := []byte(d.input)
        // validtrue expects index at the position after 't'
        _, ok := validtrue(data, d.startPos)
        subTt.Equal(d.valid, ok)
    })
}
...
func TestValidFalse(t *testing.T) {
    ...
    tt.RunTests(tests, func(subTt *zlsgo.TestUtil, tc zlsgo.TestCase) {
        d := tc.Data.(FalseTestCase)
        data := []byte(d.input)
        _, ok := validfalse(data, d.startPos)
        subTt.Equal(d.valid, ok)
    })
}
...
func TestParseInt(t *testing.T) {
    ...
    tt.RunTests(tests, func(subTt *zlsgo.TestUtil, tc zlsgo.TestCase) {
        d := tc.Data.(ParseIntTestCase)
        n, ok := parseInt(d.input)
        subTt.Equal(d.valid, ok)
        if ok {
            subTt.Equal(d.expected, n)
        }
    })
}
